...
//       className="fixed inset-0 z-[999] w-full h-full overflow-y-auto overflow-x-hidden text-theme-primary font-mono selection:bg-[#5b7fc7] selection:text-white transition-colors duration-500 fp-text-soft"
//     >
...
//       <div className={`fixed inset-0 backdrop-blur-md pointer-events-none -z-10 ${isDark ? 'bg-[#1c1c1c]/40' : 'bg-white/40'}`} />
...
//         {/* Right 65% */}
//         <div className="w-full lg:w-[65%] min-h-[350px] sm:min-h-[500px] lg:min-h-screen bg-theme-tertiary/50 overflow-hidden transition-colors duration-500">
//           {mainHeroImage && (
//             <img
//               src={mainHeroImage}
//               alt={project.title}
...
//             <Link to="/projects" className="underline hover:text-theme-primary">
//               Projects
//             </Link>
//             <span>→</span>
...
//               >
//                 <div className="w-full h-[280px] sm:h-[340px] lg:h-[380px] bg-theme-tertiary overflow-hidden rounded-sm transition-colors duration-500">
//                   <img
//                     src={item.image}
//                     alt={item.title}
//                     loading="lazy"
...
function StoryBlock({ title, text }) {
  ...
  return (
    <div className="space-y-3 sm:space-y-4" onMouseLeave={() => setIsExpanded(false)}>
      <h3 className="font-mono text-lg sm:text-2xl text-theme-primary font-normal leading-tight">
        {title}
      </h3>

      <div
        className={`font-mono text-sm sm:text-lg lg:text-xl font-light text-theme-muted leading-relaxed overflow-hidden transition-[max-height] duration-500 ease-in-out ${
          isExpanded
            ? "max-h-[2000px] line-clamp-none"
            : "max-h-[78px] sm:max-h-[98px] lg:max-h-[117px] line-clamp-3"
        }`}
      ...
      <button
        onMouseEnter={() => setIsExpanded(true)}
        onClick={() => setIsExpanded(!isExpanded)}
        className="inline-flex items-center gap-3 text-sm sm:text-lg font-medium text-[#5b7fc7] transition-colors cursor-pointer pt-2"
      >
        <span className="text-base sm:text-xl font-light text-neutral-400 select-none">
          {isExpanded ? "−" : "+"}
        </span>
        <span className="tracking-tight">
          {isExpanded ? "Read less" : "Read more"}
  ...
  if (!project) {
    return (
      <main className="fixed inset-0 z-[999] overflow-y-auto flex flex-col items-center justify-center font-mono px-6">
        ...
        <h2 className="text-xl sm:text-2xl font-light mb-4 text-theme-primary">
          Project Not Found
        </h2>
        <p className="text-theme-muted mb-6 max-w-md text-center text-xs sm:text-sm">
          The project entry you are looking for might have been moved or renamed.
        </p>
        <Link
          to="/projects"
          className="inline-flex items-center gap-2 text-xs uppercase tracking-widest font-bold text-[#5b7fc7] hover:underline"
        >
          ← Return to Projects Index
        </Link>
      </main>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        // style={{ backgroundImage: `url(${projectBg})` }}
      >
        <div className="absolute inset-0 backdrop-blur-[1px] pointer-events-none transition-colors duration-500" style={{ backgroundColor: 'var(--bg-primary)' }} />
      </div>

      {/* 2. Semi-Transparent Overlay + Blur */}
      <div className="fixed inset-0 backdrop-blur-md pointer-events-none -z-10 transition-colors duration-500" style={{ backgroundColor: 'color-mix(in srgb, var(--bg-primary) 40%, transparent)' }} />

      {/* Back Button */}
      <div className="absolute top-6 sm:top-10 left-4 sm:left-8 lg:left-12 z-50">
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-2 text-theme-primary hover:text-[#5b7fc7] transition-colors duration-200 text-xs sm:text-sm font-semibold drop-shadow-md cursor-pointer bg-theme-card/60 backdrop-blur-sm px-3 py-1.5 rounded-md lg:bg-transparent lg:p-0"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
      ...
      <section className="w-full min-h-screen flex flex-col lg:flex-row font-mono pt-16 sm:mb-0 -mb-100 lg:pt-0">
        {/* Left 35% */}
        <div className="w-full lg:w-[38%] flex items-center justify-start px-6 sm:px-10 lg:px-14 py-8 lg:py-12 z-10 lg:min-h-screen">                <h1 className="text-2xl sm:text-3xl lg:text-4xl font-normal text-theme-primary tracking-tight leading-tight break-words">
            {project.title}
          </h1>
        </div>
      ...
      <section className="w-full py-12 sm:py-16 lg:py-24 px-6 sm:px-10 lg:px-14 font-mono flex flex-col lg:flex-row">
        ...
        <div className="w-full lg:w-[75%] lg:pl-40">
          {/* Breadcrumb */}
          <div className="flex items-center gap-2 text-[11px] sm:text-lg mt-30 sm:mt-0 text-neutral-500 mb-6 sm:mb-8">
            <Link to="/projects" className="underline hover:text-theme-primary">
              Projects
            </Link>
            <span>→</span>
          ...
          <div className="text-lg sm:text-xl lg:text-2xl font-normal text-theme-primary leading-snug tracking-tight mb-10 sm:mb-12">
            <p>{project.description || project.summary}</p>

            {project.description && project.summary && (
              <p className="mt-6 text-sm sm:text-base lg:text-lg text-theme-muted font-light leading-relaxed">
                {project.summary}
              </p>
            )}
          </div>
          ...
          {/* Project Details Meta Grid */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 sm:gap-8 pt-8 border-t border-theme">
            <div>                <span className="block text-[10px] sm:text-xs text-theme-muted uppercase tracking-wider mb-1">
                Typologies
              </span>                <span className="text-xs sm:text-sm lg:text-base font-semibold text-theme-primary block">
                {project.category || project.type || "Restaurant"}
              </span>
            </div>
            ...
            <div className="sm:-ml-15 ml-0">                <span className="block text-[10px] sm:text-xs text-theme-muted uppercase tracking-wider mb-1">
                Location
              </span>                <span className="text-xs sm:text-sm lg:text-base font-semibold text-theme-primary block">
                {project.location || "Gulshan, Dhaka"}
              </span>
      ...
      {restGalleryImages.length > 0 && (
        <section className="w-full py-12 sm:py-16 lg:py-20 px-6 sm:px-10 lg:px-14 font-mono border-t border-theme">
          <div className="w-full">
            <h2 className="text-lg sm:text-xl lg:text-2xl font-bold uppercase tracking-widest mb-8 text-theme-primary">
              Gallery
            </h2>
            ...
            <div className="flex flex-col gap-8 lg:gap-12">
              {galleryRows.map((row, rowIdx) => (
                ...
                >
                  {row.map((img, imgIdx) => (
                    <div
                      key={imgIdx}                        className={`w-full overflow-hidden group rounded-sm bg-theme-tertiary transition-colors duration-500 ${
                          rowIdx === 0
      ...
      <section className="w-full py-16 sm:py-20 lg:py-24 px-6 sm:px-10 lg:px-14 font-mono border-t border-theme">
        <div className="w-full">
          <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4 mb-10 sm:mb-14">
            <h2 className="text-2xl sm:text-4xl md:text-5xl lg:text-4xl mb-2 sm:mb-4 font-normal font-mono tracking-tight sm:tracking-[0.1rem] leading-none uppercase text-theme-primary">
              More Projects
            </h2>
            <Link
              to="/projects"
              className="group inline-flex items-center gap-3 sm:mb-10 mb-0 text-xs sm:text-base lg:text-lg font-bold text-theme-primary tracking-wider hover:tracking-widest hover:text-[#5b7fc7] transition-all duration-300 ease-out"
            >
              <span>View All Projects</span>
              <span className="inline-block transition-transform duration-300 ease-out group-hover:translate-x-3">
                →
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 sm:gap-10 items-start">
            {moreProjects.map((item) => (
              ...
              >
                ...
                <div className="flex flex-col gap-2 pt-2">
                  <div className="text-lg sm:text-xl lg:text-2xl font-semibold leading-tight text-theme-primary group-hover:text-[#5b7fc7] transition-all duration-300 ease-out">
                    <span className="inline-block transition-all duration-300 ease-out group-hover:tracking-wider active:tracking-widest">
                      {item.title}
                    </span>
                    {item.summary && (
                      <span className="text-theme-muted font-light block text-sm sm:text-base mt-1">
                        {item.summary}
                      </span>
                    )}
                  </div>
                  ...
                  <p className="text-xs sm:text-sm text-[#5b7fc7] font-mono font-semibold">
                    {item.year}
                  </p>
                </div>
              </Link>
